...
class TrajectoryBalance:
    ...
    def compute_batch_losses(self, model, batch):
        ...
        # we want to compute n_loss = (l(s_i) - sum_{j=0}^{i-1}[ log(n(s_j)) - log(n(s_j+1)) ])^2
        # as the PCL loss (with s_0 term being 0), where l(s_i) is our predicted value.
        # batch.log_p_B contains expected value of -[ log(n(s_j)) - log(n(s_j+1) ], since it is 1 / #parents
        # and i guess we are calling that good enough.
        # scatter can then be used to sum the (negated) targets, and then we compute loss with
        # final_graph_idxs - 2, to get the graph before the stop action
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        #val_src = DataSource(ctx, algo, task, is_algo_eval=True)
        #val_src.do_sample_model_n_times(model, 64, num_total=10*64)
        #valid_dl = torch.utils.data.DataLoader(val_src, batch_size=None)
